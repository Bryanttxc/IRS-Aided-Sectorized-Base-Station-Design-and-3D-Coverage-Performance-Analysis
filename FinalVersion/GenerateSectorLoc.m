...
for p=1:NumSites
    ...
    plot([Sites(p,1) Sites(p,1)+r*cosd(60)],[Sites(p,2) Sites(p,2)+r*sind(60)],'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(60)]-1.5*r,[Sites(p,2) Sites(p,2)+r*sind(60)]+sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(60)]+1.5*r,[Sites(p,2) Sites(p,2)+r*sind(60)]-sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(60)]-1.5*r,[Sites(p,2) Sites(p,2)+r*sind(60)]-sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(60)],[Sites(p,2) Sites(p,2)+r*sind(60)]-sqrt(3)*r,'-b')

    %%%plot parallel lines: 180 degree lines
    plot([Sites(p,1) Sites(p,1)+r*cosd(180)],[Sites(p,2) Sites(p,2)+r*sind(180)],'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(180)],[Sites(p,2) Sites(p,2)+r*sind(180)]+sqrt(3)*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(180)],[Sites(p,2) Sites(p,2)+r*sind(180)]-sqrt(3)*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(180)]+1.5*r,[Sites(p,2) Sites(p,2)+r*sind(180)]+sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(180)]+1.5*r,[Sites(p,2) Sites(p,2)+r*sind(180)]-sqrt(3)/2*r,'-b')
    
    %%%plot parallel lines: 300 degree lines  
    plot([Sites(p,1) Sites(p,1)+r*cosd(300)],[Sites(p,2) Sites(p,2)+r*sind(300)],'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(300)]-1.5*r,[Sites(p,2) Sites(p,2)+r*sind(300)]-sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(300)]+1.5*r,[Sites(p,2) Sites(p,2)+r*sind(300)]+sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(300)]-1.5*r,[Sites(p,2) Sites(p,2)+r*sind(300)]+sqrt(3)/2*r,'-b')
    plot([Sites(p,1) Sites(p,1)+r*cosd(300)],[Sites(p,2) Sites(p,2)+r*sind(300)]+sqrt(3)*r,'-b')
    ...
    text(Sites(p,1)+0.75*r,Sites(p,2),sprintf('%d',3*(p-1)+1),'FontSize',15);
    text(Sites(p,1)-0.75*r,Sites(p,2)+0.75*r,sprintf('%d',3*(p-1)+2),'FontSize',15);
    text(Sites(p,1)-0.75*r,Sites(p,2)-0.75*r,sprintf('%d',3*(p-1)+3),'FontSize',15);
end
axis equal tight
...
%%% sign: site %%%
hold on
plot(Sites(1,1)+1.4*r,Sites(1,2)+0.3*r,'k^','MarkerFaceColor','r','MarkerSize',10);
text(Sites(4,1)-2.5*r,Sites(4,2)+0.65*r,sprintf('site'),'FontSize',20,'Color','r')
